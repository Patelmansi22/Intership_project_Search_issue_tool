...
setData1(data)
  ...
  useEffect(() => {
    setIsEdit(true)
    let s = data1?.filter((item)=>item.Employee_Details[0]._id==id)
  
    setEmail(s[0]?.Employee_Details[0].email)
    setName(s[0]?.Employee_Details[0].name);
    setEmpId(s[0]?.Employee_Details[0].employee_id);
    setTechnology(s[0]?.Employee_Details[0].technology);
    setProjectmanagername(s[0]?.Employee_Details[0].projectmanagername);
    setCompanyName(s[0]?.Employee_Details[0].companyname) 
